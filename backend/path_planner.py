...
def is_node_safe(gas_data):
    if not gas_data:
        return True
    if gas_data['h2s'] >= Config.H2S_THRESHOLD:
        return False
    if gas_data['ch4'] >= Config.CH4_THRESHOLD:
        return False
    return True
...
def calculate_node_risk(gas_data):
    if not gas_data:
        return 0.0
    h2s_risk = gas_data['h2s'] / Config.H2S_THRESHOLD if Config.H2S_THRESHOLD > 0 else 0
    ch4_risk = gas_data['ch4'] / Config.CH4_THRESHOLD if Config.CH4_THRESHOLD > 0 else 0
...
def dijkstra_safe_route(start_id, end_id):
    ...
    start_risk = calculate_node_risk(gas_readings.get(start_id))
    if not is_node_safe(gas_readings.get(start_id)):
        return {
            'success': False,
            'error': f'起点 {start_id} 气体浓度超标，无法出发'
        }
    if not is_node_safe(gas_readings.get(end_id)):
        return {
            'success': False,
            'error': f'终点 {end_id} 气体浓度超标，无法到达'
        }
    ...
    while pq:
        ...
        if current == end_id:
            ...
            for nid in path:
                ...
                if gas:
                    detail['h2s'] = gas['h2s']
                    detail['ch4'] = gas['ch4']
                    detail['recorded_at'] = gas['recorded_at'].isoformat() if gas['recorded_at'] else None
                    total_h2s += gas['h2s']
                    total_ch4 += gas['ch4']
        ...
        for neighbor, distance in graph[current]:
            ...
            if not is_node_safe(neighbor_gas):
                continue
    ...
    return {
        'success': False,
        'error': f'无法找到从 {start_id} 到 {end_id} 的安全路线，所有路径均经过超标节点'
    }
...
def get_all_nodes_status():
    ...
    for node in nodes:
        ...
        if gas:
            ...
            entry['recorded_at'] = gas['recorded_at'].isoformat() if gas['recorded_at'] else None
        else:
            entry['h2s'] = 0.0
            entry['ch4'] = 0.0
